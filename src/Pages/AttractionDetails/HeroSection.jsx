  ...
  return (
    <>
      <MetaData
        ...
        link_title={excursion?.title}
        description={"Get best deals on Travel and Tourism online with us"}
        thumbnail={excursion?.images ? excursion?.images[0] : ""}
      />
            ...
            <div className="relative lg:grid lg:grid-cols-12 gap-5 py-2 lg:my-0 lg:py-5">
              <div className="1st lg:col-span-8">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="text-xs text-text   flex space-x-1 items-center">
                        <div className="">
                          <button className="bg-yellow-500 w-16 px-2 py-1 text-light rounded-md capitalize">
                            {excursion?.bookingType}
                          </button>
                        </div>
                        <div className="flex space-x-3 items-center">
                          {excursion?.cancellationType ===
                            "freeCancellation" && (
                          ...
                          {excursion?.isOffer === true &&
                            excursion?.offerAmountType === "flat" && (
                              <span className="text-light bg-green-600 w-20 py-1 whitespace-nowrap text-center rounded-md capitalize text-xs">
                                {excursion?.offerAmountType === "flat"
                                  ? `$ ${excursion?.offerAmount} OFF`
                                  : ""}{" "}
                              </span>
                            )}
                          {excursion?.isOffer === true &&
                            excursion?.offerAmountType === "percentage" && (
                              <span className="text-light bg-green-600 w-20 py-1 whitespace-nowrap text-center rounded-md capitalize text-xs">
                                {excursion?.offerAmountType === "percentage"
                                  ? `${excursion?.offerAmount} % OFF`
                                  : ""}{" "}
                      ...
                      <div className="flex items-center space-x-3 text-sm">
                        <span className=" text-yellow-500 flex space-x-1 ">
                          <Rating
                            value={excursion?.averageRating}
                            text={excursion?.totalReviews}
                            color={"#FED049"}
                          />
                        </span>
                ...
                <div className="md:my-5 mx-2 lg:mx-0">
                  <>
                    <div
                      id="availability"
                      className="p-5 mb-2 lg:mb-5  bg-gray-300 text-darktext rounded-2xl "
                    >
                      <Availablity />
                    </div>
              ...
              <div className="2nd lg:col-span-4">
                ...
                <div
                  className={`${
                    viewBookCard
                      ? "fixed bottom-0 max-h-[93vh] overflow-y-auto w-full z-10 bg-white"
