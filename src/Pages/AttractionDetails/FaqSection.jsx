...
function FaqSection() {
    ...
    if (excursion?.faqs && excursion?.faqs.length <= 0) return null
    return (
        <div>
            <div className='py-5'>
                <div className=' my-2 text-xl text-darktext font-bold tracking-wider px-5'>{excursion?.title} FAQs</div>
                <div className='bg-light text-darktext font-light cursor-pointer rounded-2xl'>
                    {excursion?.faqs?.map((item, index) => (
