...
function BlogShareModal({ setShareModal, shareModal }) {
  ...
  return (
    <>
        <div className={`fixed lg:top-[20%] ${shareModal ? "bottom-0 " : "-bottom-full"} left-0 right-0 max-h-[50vh]  lg:flex lg:justify-center lg:items-center z-20 transition-all duration-500`} onClick={() => setShareModal(!shareModal)}>
          ...
          <div className={`${!shareModal ? "hidden" : ""} bg-white rounded-2xl lg:w-[25em]`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className='p-5 flex items-center space-x-1 font-semibold text-lg'>
              <img src={banner} alt="banner" className='object-cover h-32 w-32 rounded-2xl ' />
