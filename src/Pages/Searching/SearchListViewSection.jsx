      ...
      {excursions?.attractions?.data?.length > 0 && (
        <div className="md:grid md:grid-cols-2 gap-5">
          {excursions?.attractions?.data[0] &&
            excursions?.attractions?.data?.map((item, index) => (
              <div
                key={index}
                className="h-full snap-start mt-2 bg-light shadow-md p-3 rounded-3xl cursor-pointer mx-2 md:mx-0"
                onClick={() => {
      ...
      <>
        {excursions?.attractions?.data?.length < 1 && (
          <div className="p-6 shadow-lg rounded-lg bg-gray-100 text-gray-700">
            ...
            <p>
              The data that you are seeking is currently unavailable right now.
              We will be progressing on that. It will be found as soon as
              possible.
            </p>
            ...
            <form onSubmit={submitHandler}>
              <input
                ...
                className="outline-none text-darktext border-lightblue placeholder:text-text border text-sm py-2 rounded-md px-2 w-[20em] "
