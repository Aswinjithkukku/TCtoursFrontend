...
function SearchingResultPage() {
   ...
   const { excursionAll } = useSelector((state) => state.excursion);

   const maxValue = excursionAll?.attractions?.data?.reduce(
      (initValue, curValue) =>
         Math.max(initValue, curValue?.activity?.lowPrice),
      0
   );
   ...
   const [range, setRange] = useState(maxValue);
