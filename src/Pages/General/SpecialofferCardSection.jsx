                        ...
                        <div className='  h-[23em] group cursor-pointer'>
                            <div className=' relative '>
                                <div className='overflow-hidden lg:rounded-xl bg-bottom'>
                                    <img className='group-hover:scale-110 object-cover  h-[23em] w-full transition-all duration-500 cursor-pointer' src={home && home?.cards?.length > 0 && process.env.REACT_APP_SERVER_URL + home?.cards[0]?.backgroundImage} alt='special offer' />
                                </div>
                                <div className='absolute top-10 px-10'>
                                    <div className=' pt-5'>
                                        <span className='bg-main text-light py-[8px] px-3'>
                                            {home && home?.cards?.length > 0  && home?.cards[0]?.tag}
                                        </span>
                                    </div>
                                    <div className='text-3xl text-darktext font-medium pt-5'>
                                        {home && home?.cards?.length > 0  && home?.cards[0]?.title}
                                    </div>
                                    <div className='text-sm text-darktext pt-3'>
                                        <p className=''>{home && home?.cards?.length > 0 && home?.cards[0]?.description}</p>
                                    </div>
