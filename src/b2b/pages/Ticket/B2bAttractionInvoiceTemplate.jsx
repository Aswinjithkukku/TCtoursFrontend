            ...
            <tbody className="text-[16px]">
              <tr className=" bg-white">
                <td className="p-2 pl-4 border-solid  border-r-[1px] text-left flex flex-col">
                  ...
                  <span>Adult : {data?.activities?.adultsCount}</span>
                  {data?.activities?.childrenCount > 0 && (
                    <span>Child : {data?.activities?.childrenCount}</span>
                  )}
                  {data?.activities?.infantCount > 0 && (
                    <span>Infant : {data?.activities?.infantCount}</span>
                  )}
                </td>
                <td className="p-2 ">
                  <div className="flex flex-col gap-y-1">
                    {data?.activities?.adultTickets?.length > 0 && (
                      <span>
                        Adult : {data?.activities?.adultTickets?.[0]?.cost} AED
                      </span>
                    )}
                    {data?.activities?.childTickets?.length > 0 && (
                      <span>
                        Adult : {data?.activities?.childTickets?.[0]?.cost} AED
                      </span>
                    )}
                    {data?.activities?.infantTickets?.length > 0 && (
                      <span>
                        Adult : {data?.activities?.infantTickets?.[0]?.cost} AED
                      </span>
                    )}
                    <span> Total : {data?.activities?.grandTotal} AED</span>
                  </div>
                </td>
              </tr>
              {data?.activities?.transferType !== "without" && (
                <tr className="text-center bg-white border-t-[1px]">
                  <td className="p-2 pl-4 border-solid  border-r-[1px] text-left flex flex-col">
                    <span className="capitalize">
                      Transfer Type : {data?.activities?.transferType}
                    </span>
                  </td>
                  <td className="p-2 ">{data?.activities?.amount} AED</td>
                </tr>
              )}
            </tbody>
          </table>
