  ...
  const handleSubmit = async (e) => {
    try {
      ...
      if (data.email !== data.confirmEmail) {
        ...
        return setError('Email you have entered is not similiar')
      } else if (data.password !== data.confirmPassword) {

        return setError('password you have entered is not similiar')

      } else {

        setIsLoading(true);

        const response = await axios.post("/b2b/signup", data);
          ...
          <div className='1 col-span-7 text-darktext space-y-2 text-sm'>
            ...
            <ol className="relative  border-l border-gray-200 ">
                  ...
                  <div className=' flex space-x-1'>
                    <div className='w-3/12'>
                      ...
                      <select className='select'
                        ...
                        <option>0</option>
                        {countries?.map((item, index) => (
                          <option className='' value={item?._id} key={index}>
              ...
              <li className={`mb-10 ml-6 ${register.password ? 'h-full' : 'h-8'} overflow-hidden `}>
                ...
                <div className='pt-3'>
                  <button className='text-sm font-medium text-light bg-lightblue w-[100px] py-2 rounded'>Signup</button>
                </div>
              </li>
            </ol>
