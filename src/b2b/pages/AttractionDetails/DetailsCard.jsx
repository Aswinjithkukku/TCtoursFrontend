...
function DetailsCard() {
    ...
    const { agentExcursion, agentExcursionCart } = useSelector(state => state.agentExcursions)
        ...
        if (agentExcursion?.activities) {
            if (agentExcursion?.isOffer) {
                if (agentExcursion?.offerAmountType === "flat") {
                    let offer = agentExcursion?.activities[0]?.adultPrice - agentExcursion?.offerAmount
                    setOfferAmount(offer)
                } else {
                    let offer = agentExcursion?.activities[0]?.adultPrice - ((agentExcursion?.activities[0]?.adultPrice * agentExcursion?.offerAmount) / 100)
                    setOfferAmount(offer)
                }
            } else {
            ...
            <div className='bg-light  lg:rounded-xl p-5 space-y-2 mb-2'>
                ...
                <div className=''>
                    {agentExcursion?.isOffer && (
                        <p className='text-main text-xs'><s>{priceConversion(agentExcursion?.activities && (agentExcursion?.activities[0]?.adultPrice), selectedCurrency, true)}</s></p>
                    )}
                </div>
                <div className='flex justify-between items-center'>
                    ...
                    {/* offer percentage  */}
                    {agentExcursion?.isOffer && (
                        <span className='bg-soft px-3 py-2 rounded-full text-blue'>{priceConversion(agentExcursion?.offerAmount && agentExcursion?.offerAmount, selectedCurrency, false)} {agentExcursion?.offerAmountType && agentExcursion?.offerAmountType === "flat" ? selectedCurrency?.isocode : "%"} OFF</span>
                    )}
                </div>
            </div>
                            ...
                            <div className=''>
                                {(agentExcursion?.isOffer === true) && (
                                    <div className='flex justify-between text-darktext'>
                                        <span className=''>{agentExcursion?.offerAmountType === 'flat' ? 'flat' : "discount"} </span>
                                        <span className=''>{agentExcursion?.offerAmount + "%"} </span>
                                    </div>
                                )}
                                {/* <div className='flex justify-between text-darktext'>
