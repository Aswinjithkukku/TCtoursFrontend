...
function BottomNav() {
  ...
  return (
    <>
      <div className={`${location.pathname.includes('/b2b/attractions/details/') ? 'hidden' : 'block'} lg:hidden fixed w-full bottom-0 bg-primaryColor text-white h-[60px] flex justify-center items-center`}>
        <div className='grid grid-cols-5 gap-4 px-2'>
          ...
          <div className=' flex justify-center items-center relative'>
            <div className='absolute -top-9 w-[70px] h-[70px] bg-main rounded-full flex justify-center items-center'
      ...
      {searchModal && (
        <>
          <div className='lightglass absolute top-0 left-0 right-0 bottom-0 z-10' onClick={() => setSearchModal(false)}>
          </div>
          <SearchCardModal setSearchModal={setSearchModal} searchModal={searchModal} />
        </>
      )}
    </>
  )
