...
function CartModal({ setCart }) {
  ...
  return (
    <div className="absolute z-20 top-7 md:top-10 right-0 bg-[#EFF5F5] rounded-xl w-[400px]  max-h-[80vh] overflow-y-auto"  >
      <div className=" p-2">
        <div className='border rounded-lg overflow-hidden'>
          ...
          {excursionCart?.map((item, index) => (
            <div className="  hover:bg-gray-100 p-2  border-b  overflow-hidden" key={index}>
              ...
              <div className='flex justify-end'>
                <p className='text-[15px] font-[600] text-start whitespace-nowrap capitalize'>{priceConversion(item?.price, selectedCurrency, true)}</p>
              </div>
            </div>
          ))}
        </div>
